...
import Orchard.Poseidon.Pow5
import Orchard.Poseidon.Sponge
...
- `Orchard.Poseidon.Pow5.Constants` contains the concrete Pallas constants from
  `halo2_poseidon/src/fp.rs`.
- `Orchard.Poseidon.Pow5` contains the `Pow5Chip` gate-level and entry-level
  circuits from `poseidon/pow5.rs`.
- `Orchard.Poseidon.Sponge` is reserved for `poseidon.rs` sponge helpers and
  `PoseidonSpongeInstructions`-level APIs.
- `Orchard.Poseidon.Hash` is reserved for the source-level `Hash::init` and
  `Hash::hash` APIs.
